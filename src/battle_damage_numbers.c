...
static u8 GetDamageNumberPalette(u8 battler)
{
    u16 flags = gBattleStruct->moveResultFlags[battler];
	s16 passiveHp = gBattleStruct->passiveHpUpdate[battler];

	if (passiveHp < 0)
	{
		return 1; // healing - green
	}
	else if (DN_CONFIG_EFFECTIVENESS_PALETTE && (flags & MOVE_RESULT_SUPER_EFFECTIVE))
	{
		return 2; // super effective - orange	
	}
	else if (DN_CONFIG_EFFECTIVENESS_PALETTE && (flags & MOVE_RESULT_NOT_VERY_EFFECTIVE))
	{
...
void ShowDamageNumbers(u8 battler)
{
	...
	s16 dmg = gBattleStruct->moveDamage[battler];
	if (dmg == 0)
        dmg = gBattleStruct->passiveHpUpdate[battler];
	
	LoadSpritePalette(&sDamageNumSpritePalette_Normal);
	palette = GetDamageNumberPalette(battler);
	
	damage = (dmg < 10000) ? dmg : 9999; // damage is capped at 10,000 anyway so just assume 4 digits
