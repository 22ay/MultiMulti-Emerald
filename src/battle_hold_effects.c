...
enum ItemEffect TryBoosterEnergy(u32 battler, ActivationTiming timing)
{
    ...
    else if (BattlerHasTrait(battler, ABILITY_QUARK_DRIVE) && !(GetAttackerTerrain(battler, GetTerrain()) & BATTLE_FIELD_ELECTRIC_TERRAIN))
...
{
    if (GetAttackerTerrain(battler, GetTerrain()) & terrainFlag && CompareStat(battler, statId, MAX_STAT_STAGE, CMP_LESS_THAN))
    {
        gEffectBattler = gBattleScripting.battler = battler;
...
static enum ItemEffect TryKingsRock(u32 battlerAtk, u32 battlerDef)
{
    ...
    if (gSideStatuses[GetBattlerSide(battlerAtk)] & SIDE_STATUS_RAINBOW && gCurrentMove != MOVE_SECRET_POWER)
        holdEffectParam *= 2;
    if ((!BattlerHasTrait(battlerAtk, ABILITY_STENCH) && !BattlerHasTrait(battlerAtk, ABILITY_UNNERVE)) && RandomPercentage(RNG_HOLD_EFFECT_FLINCH, holdEffectParam))
    {
...
static enum ItemEffect TryRockyHelmet(u32 battlerDef, u32 battlerAtk)
{
    ...
    if (IsBattlerTurnDamaged(battlerDef)
     ...
     && !IsAbilityAndRecord(battlerAtk, ABILITY_INDOMITABLE)
